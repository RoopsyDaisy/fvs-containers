...
#
# This is the DEV environment, not a deliverable. The shippable images are the
# webgui and cluster targets of the single multi-target docker/Dockerfile.
#
...
# Toolchain + FVS runtime libs and dev conveniences. The fvsOL R package set is
# NOT installed here; it is restored from renv.lock (pinned P3M snapshot) by
# postCreate. We install the *system* libraries those R packages need at runtime
# (derived via pak::pkg_sysreqs for ubuntu-24.04), since renv installs R binaries
# but not their apt dependencies. (The old Python/uv batch tooling was pruned —
# this repo is R-only now — so python3 and uv are no longer installed here.)
